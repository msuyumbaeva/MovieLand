...
namespace MovieLand.BLL.DataAnnotaions
{
    // <summary>
    ...
    public sealed class FileSizeAttribute : ValidationAttribute
    {
        ...
        public FileSizeAttribute(int maximumSize) : base() {
        ...
        public override string FormatErrorMessage(string name) {
            ...
            string errorMessage = MinimumSize != 0 ? GetMinimumValidationErrorMessage() : GetMaximumValidationErrorMessage();
        ...
        private void EnsureLegalSizes() {
            if (MaximumSize < 0) {
                throw new InvalidOperationException(GetMaximumValueInvalidErrorMessage());
            }

            if (MaximumSize < MinimumSize) {
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, GetRangeValidationErrorMessage(), MaximumSize, MinimumSize));
            }
        }

        private string GetMinimumValidationErrorMessage() {
            return $"Minimum allowed file size is { MinimumSize } bytes.";
        }

        private string GetMaximumValidationErrorMessage() {
            return $"Maximum allowed file size is { MaximumSize } bytes.";
        }

        private string GetMaximumValueInvalidErrorMessage() {
            return $"Maximum file size must be greater or equal to 0.";
        }

        private string GetRangeValidationErrorMessage() {
            return $"Minimum value of range must be less then maximum value.";
        }
    }
}
